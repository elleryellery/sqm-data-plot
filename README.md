...
SQM devices measure sky quality in units of **MSAS** (magnitudes per square arcsecond). Higher MSAS values indicate a darker sky, while lower MSAS values indicate a brighter sky. Daytime values usally reach MSAS values of less than zero, but SQM devices report negative values as 0.0 MSAS.

## Using SDP directly

1. Download SDP and ensure that dependencies are installed.
    - To download: [click here](https://github.com/elleryellery/sqm-data-plot/archive/refs/heads/SQM-Prediction.zip), extract ZIP folder, and run Python files in the terminal or your code editor of choice.
    - Dependencies required: **astral, ephem, requests, matplotlib, scipy**. These can be installed using the command `pip install astral ephem requests matplotlib scipy` in your terminal (in some code editors, it may be necessary to precede the command with `py -m`).
2. Run `driver.py`. When prompted to upload files, upload your dataset of .dat files. You may select multiple files, and example data can be found in the `data` folder. SDP automatically provides terminal prompts that will run frequently used commands. Follow the terminal prompts to generate your desired graph.
...
2. Run the `read_file()` method from `driver.py` to upload and interpret SQM data files and populate the dataset with .dat files. 
...
- `markers-individual`: Graphs sun and moon event markers (i.e. dusk, dawn, sunrise, sunset, moonrise, and moonset) for a single specified date. Includes text displaying the moon illumination and date. **Note that this is not an especially useful graph on its own and is intended to be overlaid on `raw-individual`.**
...
- `%date={date}`: Specifies the date to graph in YYYY/MM/DD format for command bases that require a specified date (i.e. commands that do not use the entire dataset). Example: `weather%date=2026/07/10`
...
- `%filter={filter}`: Two of the base commands allow the user to filter out bad data. Both data filters default to off.
    - For `raw-all`: Filter is a **string**. `'by value'` removes data points where MSAS is below the threshold `graph.msas_night_threshold` (can be set/specified by user within the code, defaults to 17.0). `'by dawn/dusk'` removes values whose timestamps are *not* between dusk and dawn. `'none'` is the default and applies no filter. Example: `%filter=by value`
    - For `sinusoidal`: Filter is a **boolean value**. When set to `True`, data points from nights with a bright moon will be removed. Example: `%filter=True`.
...
## Graph settings
Unlike qualifiers, which can be used on each individual dataset before it gets overlaid, settings are used for an entire graph. Settings are defined by the `&` key and should be applied before each `SPLIT` keyword.
...
- `&sharex` makes all graphs share the same x-axis scaling. This will apply to the current graph and all graphs preceding it in the command (where graphs are defined by the `SPLIT` keyword).
    - The images below compare the same graph with and without the sharex setting. The bottom graph was generated with the command `raw-individual%date=2026/07/03&invert&grid SPLIT weather%date=2026/07/03&grid&sharex`.
      <img src="./img/sharex-off.png" alt="Graph without sharex enabled">
      ^ Time axis is not aligned!
        <img src="./img/sharex-on.png" alt="Graph with sharex enabled">
...
The repeat command should be called only once per graph and should be placed after all settings and qualifiers.
...
This command will create 5 different graphs, spanning from 7/21/2026-7/25/2026, each with `markers-individual` overlaid on `prediction`.
  ...
  <img src="./img/repeat-example-s.png" alt="Example graph generated using #repeat in mode s">
...
This command will show 7 days of data for the week beginning on 7/21/2026. All of the predictions will be shown on the same graph, while all of the weather data will be shown on a separate graph, as seen below.
  ...
  <img src="./img/repeat-example-o.png" alt="Example graph generated using #repeat in mode o">
